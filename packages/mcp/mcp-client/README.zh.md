...
| `cwd` | stdio | 否 | 子进程工作目录 |
| `url` | http、sse | 是 | MCP 服务器 URL（`"sse"` 时为 SSE 端点） |
| `headers` | http、sse | 否 | 额外标头（例如认证 token），附加到 HTTP 传输的双向请求 |
...
对于在 `TraceTelemetry` 下运行的 HTTP 传输，每次 MCP HTTP 尝试都会在活动 `mcp.tools.call` Span 下创建一个 `mcp.client` Span 并注入其本地 W3C 上下文。Harness 不读取返回的 trace 或 request 标识；MCP 重试绝不会退回为未追踪请求。
