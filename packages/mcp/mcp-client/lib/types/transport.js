 ...
 */
export function traceAwareFetch(telemetry, input, init) {
 ...
 */
export function createTransport(config, telemetry) {
    switch (config.transport) {
        ...
        case 'streamable-http':
            ...
            return new StreamableHTTPClientTransport(new URL(config.url), { requestInit: { headers: config.headers }, fetch: (input, init) => traceAwareFetch(telemetry, input, init) });
        case 'sse':
            ...
            return new SSEClientTransport(new URL(config.url), { requestInit: { headers: config.headers }, fetch: (input, init) => traceAwareFetch(telemetry, input, init) });
    }
}
