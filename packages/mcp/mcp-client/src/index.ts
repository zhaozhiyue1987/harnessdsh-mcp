 ...
 */
const liveConnections = new WeakMap<Context, Map<string, ConnectionHandle>>()
 ...
 */
export function connectionHandle(ctx: Context, serverName: string): ConnectionHandle | undefined {
...
export async function apply(ctx: Context, config: Config): Promise<void> {
  ...
  // Publish the handle so the management surface can read the current tool
  // inventory; registration is an effect, so disposal unregisters it.
  ctx.effect(() => {
