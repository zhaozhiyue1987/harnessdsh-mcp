...
import type { McpToolDescriptor, ToolBridgeOptions, ToolDisposers } from './tools.ts'
...
export interface ConnectionHandle {
  ...
  /**
   * Snapshot of the MCP tools the server currently advertises — the last
   * fetch the supervisor synced, cleared when the server tears down. Useful
   * for programmatic consumers (e.g. a management UI) without reaching into
...
export function startConnection(ctx: Context, config: Config, policy: ResolvedReconnectPolicy): ConnectionHandle {
  ...
  let liveTools: readonly McpToolDescriptor[] = []
  ...
  function enqueueSync(generation: Client, syncOpts: ToolBridgeOptions = opts): Promise<void> {
    const run = syncChain.then(async () => {
      ...
      disposers = await syncTools(generation, ctx, syncOpts, disposers, (tools) => {
        // A stale generation that finished issuing its sync before the current
        // one took over must not overwrite the live list.
